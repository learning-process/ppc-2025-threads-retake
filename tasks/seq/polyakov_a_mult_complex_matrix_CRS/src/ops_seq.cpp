...
polyakov_a_mult_complex_matrix_CRS_seq::MatrixCRS polyakov_a_mult_complex_matrix_CRS_seq::GetRandomMatrixCRS(size_t n, size_t m, size_t sparsity_coeff) {
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    for (size_t j = 0; j < m; j++) {
      if(try_dist(gen) <= sparsity_coeff) {
        //std::complex<double> compl(real_dist(gen), imag_dist(gen));
        //values.push_back(compl);
...
bool polyakov_a_mult_complex_matrix_CRS_seq::TestTaskSequential::ValidationImpl() {
  // Возможно ли умножение матриц (сравнение строк и столбцов,) не нулевые n m p,
