...
TEST(strakhov_a_double_radix_merge_seq, test_pipeline_run) {
  ...
  task_data_seq->inputs.emplace_back(reinterpret_cast<uint64_t *>(in.data()));
  task_data_seq->inputs_count.emplace_back(in.size());
  task_data_seq->outputs.emplace_back(reinterpret_cast<uint64_t *>(out.data()));
  task_data_seq->outputs_count.emplace_back(out.size());

  // Create Task
  auto test_task_sequential = std::make_shared<strakhov_a_double_radix_merge_seq::DoubleRadixMergeSeq>(task_data_seq);
...
TEST(strakhov_a_double_radix_merge_seq, test_task_run) {
  constexpr int kCount = 500;
