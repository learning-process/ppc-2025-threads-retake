...
static std::vector<double> RunTask(const std::vector<double>& input) {
  ...
  RadixBatcherSortSequential task(task_data_seq);
...
TEST(kavtorev_d_batcher_sort_seq, handles_empty) {
  std::vector<double> in;
  auto out = RunTask(in);
  EXPECT_TRUE(out.empty());
}

TEST(kavtorev_d_batcher_sort_seq, handles_single) {
...
TEST(kavtorev_d_batcher_sort_seq, sorts_already_sorted) {
  std::vector<double> in{-5.0, -1.0, 0.0, 0.5, 2.0};
  auto out = RunTask(in);
  EXPECT_EQ(out, in);
}

TEST(kavtorev_d_batcher_sort_seq, sorts_reverse_sorted) {
...
TEST(kavtorev_d_batcher_sort_seq, sorts_with_duplicates) {
...
TEST(kavtorev_d_batcher_sort_seq, sorts_negatives_and_positives) {
...
TEST(kavtorev_d_batcher_sort_seq, sorts_with_infinities) {
  std::vector<double> in{-std::numeric_limits<double>::infinity(), -1.0, 0.0, 1.0, std::numeric_limits<double>::infinity()};
  auto out = RunTask(in);
  EXPECT_EQ(out, in);
}

TEST(kavtorev_d_batcher_sort_seq, sorts_with_nan_moved_last) {
...
TEST(kavtorev_d_batcher_sort_seq, sorts_random_small) {
  std::mt19937 gen(42);
  ...
  for (size_t i = 0; i < in.size(); ++i) EXPECT_DOUBLE_EQ(out[i], in[i]);
}

TEST(kavtorev_d_batcher_sort_seq, sorts_power_of_two) {
...
TEST(kavtorev_d_batcher_sort_seq, sorts_large_random) {
  std::mt19937 gen(7);
...
TEST(kavtorev_d_batcher_sort_seq, file_driven_small) {
  std::string line;
  ...
  for (size_t i = 0; i < count; ++i) in[i] = static_cast<double>(count - i);
  auto out = RunTask(in);
  std::sort(in.begin(), in.end());
  EXPECT_EQ(out, in);
}
