...
bool dudchenko_o_connected_components::TestTaskSequential::PreProcessingImpl() {
  // Предполагаем, что входные данные содержат ширину, высоту и пиксели
  auto* in_ptr = reinterpret_cast<int*>(task_data->inputs[0]);
  width_ = in_ptr[0];
  height_ = in_ptr[1];
  
  // Копируем пиксели изображения (0 - фон, 1 - объект)
  size_t pixel_count = width_ * height_;
  input_ = std::vector<int>(in_ptr + 2, in_ptr + 2 + pixel_count);

  // Выходной массив для меток компонент
...
bool dudchenko_o_connected_components::TestTaskSequential::ValidationImpl() {
  ...
  if (task_data->inputs_count[0] < 2) {
    return false;  // Должны быть как минимум ширина и высота
  }
...
void dudchenko_o_connected_components::TestTaskSequential::LabelComponents() {
  ...
  std::vector<int> parent(total_pixels + 1, 0);  // +1 для индексации с 1
  
  FirstPass(labels, parent);
  SecondPass(labels, parent);
  
  // Копируем результат в output_
  output_ = labels;
}
...
void dudchenko_o_connected_components::TestTaskSequential::FirstPass(std::vector<int>& labels, 
  ...
  for (int y = 0; y < height_; ++y) {
    for (int x = 0; x < width_; ++x) {
      ...
      if (input_[index] == 0) {  // Фон - пропускаем
        labels[index] = 0;
        continue;
      }
      ...
      if (left_label == 0 && top_label == 0) {
        ...
        // Продолжение компонента слева
        labels[index] = left_label;
      } else if (left_label == 0 && top_label != 0) {
        // Продолжение компонента сверху
        labels[index] = top_label;
      } else {
        // Слияние двух компонентов
