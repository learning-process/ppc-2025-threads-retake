...
using ersoz_b_hoare_sort_simple_merge_stl::HoareSortSimpleMergeSTL;

TEST(ersoz_b_hoare_sort_simple_merge_stl, test_pipeline_run) {
  ...
  auto task = std::make_shared<HoareSortSimpleMergeSTL>(task_data);
...
TEST(ersoz_b_hoare_sort_simple_merge_stl, test_task_run) {
  const size_t n = 5000000;
  std::vector<int> in(n);
