...
TEST(muradov_k_histogram_stretch_stl, test_pipeline_run) {
  ...
  auto task = std::make_shared<muradov_k_histogram_stretch_stl::HistogramStretchSTL>(task_data);
...
TEST(muradov_k_histogram_stretch_stl, test_task_run) {
  const int k_size = 600000;
  std::vector<int> in(k_size);
  std::vector<int> out(k_size, 0);
  std::mt19937 gen(777);
