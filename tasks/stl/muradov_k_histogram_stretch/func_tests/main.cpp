...
TEST(muradov_k_histogram_stretch_stl, stretch_small_vector) {
  std::vector<int> in{10, 20, 30, 40, 50};
  std::vector<int> out(in.size(), -1);
  auto td = MakeTaskData(in, out);
  muradov_k_histogram_stretch_stl::HistogramStretchSTL task(td);
...
TEST(muradov_k_histogram_stretch_stl, stretch_constant) {
...
TEST(muradov_k_histogram_stretch_stl, validation_invalid_range) {
...
TEST(muradov_k_histogram_stretch_stl, stretch_full_range_preserve) {
...
TEST(muradov_k_histogram_stretch_stl, output_min_zero_max_255) {
