...
bool polyakov_a_mult_complex_matrix_CRS_tbb::TestTaskTBB::RunImpl() {
  ...
  tbb::parallel_for(tbb::blocked_range<size_t>(0, a_rows), [&](const tbb::blocked_range<size_t> &range) {
    std::vector<char> local_marked(c_cols, 0);
  ...
  C->values.assign(total_nnz, std::complex<double>(0.0));
