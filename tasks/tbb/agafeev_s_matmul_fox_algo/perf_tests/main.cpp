...
TEST(agafeev_s_matmul_fox_algo_tbb, test_pipeline_run) {
  const int n = 800;
  ...
  auto test_task_tbb = std::make_shared<agafeev_s_matmul_fox_algo_tbb::MultiplMatrixTBB>(task_data);
...
TEST(agafeev_s_matmul_fox_algo_tbb, test_task_run) {
  const int n = 800;
  size_t block_size = 30;
  // Create data
  std::vector<double> in_matrix1 = CreateRandomMatrix(n * n);
