...
TEST(agafeev_s_matmul_fox_algo_tbb, matmul_9x9) {
  const int n = 9;
  size_t block_size = 3;
  // Create data
  ...
  // Create Task
  agafeev_s_matmul_fox_algo_tbb::MultiplMatrixTBB test_task(task_data);
...
TEST(agafeev_s_matmul_fox_algo_tbb, matmul_3x3) {
  const int n = 3;
  size_t block_size = 1;
  // Create data
  std::vector<double> in_matrix1 = CreateRandomMatrix(n * n);
...
TEST(agafeev_s_matmul_fox_algo_tbb, matmul_16x16) {
  const int n = 16;
  size_t block_size = 4;
  // Create data
  std::vector<double> in_matrix1 = CreateRandomMatrix(n * n);
...
TEST(agafeev_s_matmul_fox_algo_tbb, wrong_input) {
  const int n = 3;
  size_t block_size = 5;
  // Create data
  std::vector<double> in_matrix1 = CreateRandomMatrix(n * n);
