...
void agafeev_s_matmul_fox_algo_tbb::BlockMultiply(const std::vector<double> &matr_a, unsigned long row,
...
bool agafeev_s_matmul_fox_algo_tbb::MultiplMatrixTBB::PreProcessingImpl() {
  first_input_.clear();
  second_input_.clear();
  result_.clear();
...
bool agafeev_s_matmul_fox_algo_tbb::MultiplMatrixTBB::ValidationImpl() {
...
bool agafeev_s_matmul_fox_algo_tbb::MultiplMatrixTBB::RunImpl() {
  int num_blocks = static_cast<int>((size_ + block_size_ - 1) / block_size_);
  tbb::parallel_for(0, num_blocks * num_blocks, [&](int index) {
    ...
    for (int step = 0; step < num_blocks; step++) {
      int block_index = (i + step) % num_blocks;
...
bool agafeev_s_matmul_fox_algo_tbb::MultiplMatrixTBB::PostProcessingImpl() {
