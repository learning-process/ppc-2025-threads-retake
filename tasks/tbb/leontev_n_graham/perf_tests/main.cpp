...
TEST(leontev_n_graham_tbb, test_pipeline_run) {
  ...
  // Create Task
  auto graham_tbb = std::make_shared<leontev_n_graham_tbb::GrahamTbb>(task_data_tbb);
  ...
  auto perf_analyzer = std::make_shared<ppc::core::Perf>(graham_tbb);
  perf_analyzer->PipelineRun(perf_attr, perf_results);
  ppc::core::Perf::PrintPerfStatistic(perf_results);
}

TEST(leontev_n_graham_tbb, test_task_run) {
  constexpr int kCount = 2500000;
