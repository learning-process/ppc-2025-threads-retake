...
namespace malyshev_v_strassen_algorithm_tbb {

std::vector<double> StandardMultiply(const std::vector<double> &a, const std::vector<double> &b, int size);

class StrassenTbb : public ppc::core::Task {
 public:
  explicit StrassenTbb(ppc::core::TaskDataPtr task_data) : Task(std::move(task_data)) {}
