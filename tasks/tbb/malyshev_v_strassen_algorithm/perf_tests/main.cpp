...
TEST(malyshev_v_strassen_algorithm_tbb, test_pipeline_run) {
  constexpr size_t kMatrixSize = 1024;
  ...
  std::vector<double> expected = malyshev_v_strassen_algorithm_tbb::StandardMultiply(in_a, in_b, kMatrixSize);
  ...
  auto test_task = std::make_shared<malyshev_v_strassen_algorithm_tbb::StrassenTbb>(task_data);
...
TEST(malyshev_v_strassen_algorithm_tbb, test_task_run) {
  constexpr size_t kMatrixSize = 1024;
  std::vector<double> in_a = GenerateRandomMatrix(kMatrixSize);
  std::vector<double> in_b = GenerateRandomMatrix(kMatrixSize);
  std::vector<double> out(kMatrixSize * kMatrixSize, 0.0);
