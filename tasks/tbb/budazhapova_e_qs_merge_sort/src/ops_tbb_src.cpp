...
void QuickSortMergeParallel(std::vector<int>& arr, int low, int high) {
  ...
  if (high - low + 1 > min_size) {
    ...
    if (low < pi && pi + 1 < high) {
      tbb::task_group tg;
      tg.run([&] { QuickSortMergeParallel(arr, low, pi); });
      tg.run([&] { QuickSortMergeParallel(arr, pi + 1, high); });
      tg.wait();
    } else {
      if (low < pi) {
...
bool budazhapova_e_qs_merge_sort_tbb::QSMergeSortTBB::RunImpl() {
  ...
  if (!output_.empty() && output_.size() > 1) {
    ...
    arena.execute([&] { QuickSortMergeParallel(output_, 0, static_cast<int>(output_.size()) - 1); });
  }
