...
inline void strakhov_a_double_radix_merge_omp::DoubleRadixMergeOmp::FloatToNormalized(std::vector<uint64_t> &input,
                                                                                      unsigned size) {
oneapi::tbb::parallel_for(blocked_range<size_t>(0, size), [&](const blocked_range<size_t>& r){
...
oneapi::tbb::parallel_for(blocked_range<size_t>(0, num_chunks), [&](const blocked_range<size_t>& rr){
...
oneapi::tbb::parallel_for(blocked_range<size_t>(0, n, (run << 1)), [&](const blocked_range<size_t>& r){
  ...
  if (temp_ptr != temp_vector.data()) {
            oneapi::tbb::parallel_for(blocked_range<size_t>(0, n), [&](const blocked_range<size_t> &r) {
              std::memcpy(temp_vector.data() + r.begin(), temp_ptr + r.begin(),
                          (r.end() - r.begin()) * sizeof(uint64_t));
            });
